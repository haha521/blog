		...
		<div id="content_right">
			...
			<div id="dete_line">
				...
				<el-tabs tab-position="left">
					<el-tab-pane :label="kind.year + '年'" v-for="kind in essayData.kinds" :key="kind.index">
						<ul v-for="child in kind.children" :key="child.index">
							<li @click="queryClassify(kind.year, child.month)">{{child.month + '月'}}（{{child.sum}}）</li>
						</ul>
					</el-tab-pane>
				</el-tabs>
			</div>
	...
	export default {
		...
		created() {
            ...
            .then(function (response) {
                _this.essayData = response.data.data;
            })
		...
		methods: {
			...
			queryClassify(year, month) {
				this.getPage(1, year, month);
			},
			//获取页码数据
			getPage(val, year, month) {
				let _this = this;
				this.$post("/blog/essayPage", {
					module: "心情随笔模块",
					pageNo: val,
                    pageSize: 4,
					artDate: year + '-' + month
				...
				.then(function (response) {
					_this.essayData.essayPage = response.data.data.essayPage;
				})
				.catch(function (error) {
					console.log(error);
				});
