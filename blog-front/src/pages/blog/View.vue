<template>
    <div class="wrapper">
        <header>
            <div class='header'>
                <img src="static/img/blog/tou.jpg" alt="寒月饼饼的个人博客"/>
            </div>
        </header>
		<div id="content">
    ...
    export default {
        ...
        methods:{
			...
			handleCurrentChange(val){
				...
				.then(function (response) {
                    _this.viewData.msgPage = response.data.data.msgPage;
				})
				.catch(function (error) {
					console.log(error);
				});
		...
		created(){
			...
			//上下文的改变
            let _this = this;
            this.$get(`/blog/view/`+id)
            .then(function (response) {
				_this.viewData = response.data.data;
               // _this.$alert(_this.viewData.article);
