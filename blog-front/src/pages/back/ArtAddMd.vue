                            ...
                            <el-col :span="8">
                                <el-form-item label="模块选择" prop="module">
                                    <el-select placeholder="请选择" v-model="artData.module"
                                        @change="moduleChange(artData.module)">
                                        <el-option v-for="(type, index) in mtcs.modules" :key="index"
                            ...
                            <el-col :span="8">
                                <el-form-item label="所属类型" prop="type">
                                    <el-select placeholder="请选择" v-model="artData.type"
                                        @change="typeChange(artData.module,artData.type)">
                                        <el-option v-for="(type, index) in mtcs.types" :key="index"
                                            :label="type" :value="type"></el-option>
                                    </el-select>
                                </el-form-item>
                            </el-col>
                            <el-col :span="8">
                                <el-form-item label="分类选择" prop="classify">
                                    <el-select placeholder="请选择" v-model="artData.classify">
                                        <el-option v-for="(type, index) in mtcs.classifys" :key="index"
                                            :label="type.val" :value="type"></el-option>
                                    </el-select>
                                </el-form-item>
                            </el-col>
                        </el-row>
    ...
    export default {
        ...
        methods: {
            ...
            getMtcs(){
                let _this = this;
                //获取下拉选项的值
                this.$get("/kind/module")
                    .then(function (response) {
                        _this.mtcs.modules = response.data.data.modules;
                    })
                    .catch(function (error) {
                        console.log(error);
                    });
            },
            typeChange(module,type){
                // let _this = this;
                // let consts = [];
                // if (1 === type) {
                ...
                // });
                let _this = this;
                _this.mt.module = module;
                _this.mt.type = type;
                _this.artData.classify = '';
                //获取下拉选项的值
                this.$get(`/kind/classify/${module}/${type}`)
                    .then(function (response) {
                        _this.mtcs.classifys = response.data.data.classifys;
                    })
                    .catch(function (error) {
                        console.log(error);
                    });
            ...
            moduleChange(module){
                ...
                _this.artData.classify = '';
                _this.artData.type = '';
                this.$get(`/kind/type/${module}`)
                    .then(function (response) {
                        _this.mtcs.types = response.data.data.types;
                        _this.mtcs.classifys = response.data.data.classifys;
                    })
                    .catch(function (error) {
                        console.log(error);
            ...
            loadSuccess(response){
                ...
                this.$alert(this.uploadInfo.response);
            },
            //重置数据
            reset(){
                // 基础数据重置
            ...
            saveArt(state){
                ...
                _this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        // 确认图片是否上传
                         if(this.uploadInfo.response.code === 200) {
                            // 图片路径
