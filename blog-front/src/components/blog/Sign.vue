				...
				<el-row id="login_row" v-if="sginData.isLogin">
                    <center>
                        <el-input placeholder="请输入登录账号|手机号|邮箱" prefix-icon="el-icon-info"
                            v-model="sginData.loginData.userName" maxlength="20">
                        </el-input>
                        <br/><br/>
                ...
                <el-row id="register_row" v-if="sginData.isLogin === false">
                    <center>
                        <el-input placeholder="请输入注册账号    6-20字母或数字" prefix-icon="el-icon-info"
                            v-model="sginData.registerData.userName" maxlength="20"
                            :suffix-icon="sginData.registerIconData.userName"
                            @change="reCgUserCode(sginData.registerData.userName)">
                        </el-input>
        ...
        methods: {
            ...
            signSubmit(title){
                ...
                if('登录' === title){
                    let loginMsg = "";
                    //校验录入
                    if(!this.sginData.loginData.userName){
                    ...
                    if(loginMsg){
                        ...
                        .then(function (response) {
                            if(response.data.code === 200){
                                ...
                                //渲染用户信息
                                _this.sginData.user.userNickName = response.data.data.userNickName;
                                _this.sginData.user.userImgPath = response.data.data.userImgPath;
                                _this.sginData.user.userId = response.data.data.userId;
                                _this.sginData.user.userName = response.data.data.userName;
                                localStorage.setItem('token',_this.sginData.user);
                                //将登录信息放入vuex中
            ...
            validate(){
                let msg = "请输入";
                //校验是否全部必录
                if(!this.sginData.registerData.userName){
                    msg += "注册账号";
                ...
                if(msg.length === 3){
                    //检查是否全部录入正确
                    if(el_icon_success !== this.sginData.registerIconData.userName){
                        msg += "正确的注册账号";
            ...
            reCgUserCode(val){
                ...
                if(f){
                    this.sginData.registerIconData.userName = el_icon_success;
                    this.sginData.errorMsg = "";
                }else {
                    this.sginData.registerIconData.userName = el_icon_error;
